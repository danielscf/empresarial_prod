...
public class SolicitudControlador {
    ...
    public ResponseEntity<?> registrarSolicitudUsuarioEmprendedor(
        ...
        try {
            ...
            SolicitudNuevoEmprendedorDto solicitudNuevoEmprendedorDto = objectMapper.readValue(solicitudJson,
                    SolicitudNuevoEmprendedorDto.class);

            Usuario usuarioExistente = usuarioRepositorio
                    .findUsuarioByUsuarioUsuario(solicitudNuevoEmprendedorDto.getUsuario().getUsuarioUsuario());
            if (usuarioExistente != null) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("El usuario con ese nombre de usuario ya existe");
            }

            UsuarioNuevoDto nuevoUsuario = solicitudServicioImpl
                    .convertirADtoUsuario(solicitudNuevoEmprendedorDto.getUsuario());
            Usuario usuarioGuardado = usuarioServicio.guardarUsuario(nuevoUsuario);

            Emprendedor informacionEmprendedor = solicitudServicioImpl
            ...
            solicitudUsuario.setSolicitudDescripcion("Solicitud de nuevo usuario: " + usuarioGuardado.getUsuarioNombre()
                    + " " + usuarioGuardado.getUsuarioApellidoPaterno() + " "
                    + usuarioGuardado.getUsuarioApellidoMaterno());
